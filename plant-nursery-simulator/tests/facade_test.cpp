...
void testBuildAndFinalizeOrder() {
    ...
    Order order;
    order.addItem(StockItem("Soil", 10.0, nullptr));
    order.addItem(StockItem("Pot", 5.0, nullptr));

    bool success = facade->buildAndFinalizeOrder(nullptr, order);

    ASSERT_TRUE(success, "Facade should finalise the supplied order");
...
void testProcessReturn() {
    ...
    Order order;
    order.addItem(StockItem("Orchid", 25.0, nullptr));
    bool buildSuccess = facade->buildAndFinalizeOrder(nullptr, order);

    ASSERT_TRUE(buildSuccess, "Facade should finalise the supplied order");
